...
#1 Задача 1: выбрать всех пользователей, указав их id, имя и
...
#5
/* 
Задача 5: Список медиафайлов пользователей с количеством
...
SELECT
	M.id,
    M.filename AS 'Файл',
    CONCAT(U.firstname, ' ', U.lastname) AS 'Автор',
    COUNT(L.id)'Число лайков'
	FROM media M
	LEFT JOIN likes L
		ON L.media_id = M.id
	JOIN users U 
...
#6
/* 
Задача 6: Список медиафайлов пользователей, указав название
...
/* HW_04*/
/*
Задача 1: Подсчитать общее количество лайков, которые получили пользователи
младше 13 лет.
*/

SELECT 
	COUNT(L.media_id) AS 'Общее число лайков, которые получили пользователи младше 13 лет' /*подсчитать лайки за медиа, авторы которых младше 13 лет */
FROM likes L
WHERE L.media_id IN 
	(
    SELECT /*выбрать media с авторами, которым меньше 13 лет*/
		M.id
	FROM media M
    WHERE M.user_id IN
		(
        SELECT /*выбрать пользователей младше 13 лет*/
			P.user_id
		FROM profiles P
        WHERE P.birthday > "2010-05-03"
        )
    );
...
/*2.1 Решение с помощью вложенного SELECT-а*/
SELECT
( 
SELECT COUNT(L.id) 
FROM likes L
WHERE L.user_id IN 
	(
	SELECT /*выбрать пользователей женщин*/
		P.user_id
	FROM profiles P
	WHERE P.gender =  "f"
    )
) AS 'Общее число лайков, которые поставили женщины' ,
(
SELECT COUNT(L.id) 
FROM likes L
WHERE L.user_id IN 
	(
	SELECT /*выбрать пользователей мужчин*/
		P.user_id
	FROM profiles P
	WHERE P.gender =  "m"
    )
) AS 'Общее число лайков, которые поставили мужчины' ;

/*2.2 Решение с помощью RIGHT JOIN*/
SELECT
( 
SELECT COUNT(L.id) 
FROM likes L
...
SELECT 
	U.id,
    CONCAT(U.firstname, ' ', U.lastname) AS 'Имя'
FROM users U
WHERE U.id NOT IN
	(
...
SELECT 
    ...
    M.from_user_id AS 'От пользователя, ID',
    @ID AS 'Пользователю, ID' 
FROM messages AS M
WHERE (
		(M.to_user_id = @ID) 
        AND
		(M.from_user_id IN
			(
				SELECT initiator_user_id FROM friend_requests WHERE (target_user_id = @ID) AND (`status` = 'approved')
				UNION
				SELECT target_user_id FROM friend_requests WHERE (initiator_user_id = @ID) AND (`status` = 'approved')
			)
	    )
	  )
